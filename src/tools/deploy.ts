...
export function getDeployTools(config: Config): ToolDefinition[] {
  return [
    {
      name: 'gke_deploy',
      ...
      inputSchema: {
        type: 'object',
        properties: {
          user_request: { type: 'string', description: "A natural language request specifying the configuration file to deploy. e.g. 'my-app.yaml to staging'" },
        },
        required: ['user_request'],
      },
      handler: async (args: any) => {
        if (!args.user_request || args.user_request.trim() === '') {
          throw new Error("argument 'user_request' cannot be empty");
        }

        // The Go code executes a template but the template doesn't seem to use the variable.
        // We'll just return the template string as in the Go code.
